 ...
 * @since 5.1.0
 *
 * Modified to not return the SAML2 Authn Request for the SAML2 Callback
 */
@Slf4j
...
public class RedirectToServiceAction extends AbstractAction {
    ...
    protected Event finalizeResponseEvent(final RequestContext requestContext, final WebApplicationService service, final Response response) {
        ...
        if (service.getOriginalUrl().contains("/idp/profile/SAML2/Callback")) {
            requestContext.getRequestScope().put("originalUrl",
                    new URIBuilder(service.getOriginalUrl()).clearParameters().toString());
